...
# Athena override: border-radius is 0 everywhere (sharp corners).
# The Framer-derived scale below is retained for reference; every token
# resolves to 0px in the implemented site.
rounded:
  xs: 0px
  sm: 0px
  md: 0px
...
- Border radius is uniformly `0` across the site (Athena override): buttons, cards, mockup tiles, spotlight panels, inputs and icon glyphs are all sharp-cornered. The only remaining circle vocabulary is the selection/focus ring, which is a `rgba(0,153,255,0.40)` 1px ring, not a shape.
...
> **Athena override:** every radius token resolves to `0px`. The scale below
> documents the Framer-derived values for reference; the implemented site
> applies sharp corners (`border-radius: 0`) to all surfaces.

| Token | Value | Use |
|---|---|---|
| `{rounded.xs}` | 0px | Small chip / utility radius |
| `{rounded.sm}` | 0px | Inline tag, badge |
| `{rounded.md}` | 0px | Form input, list item |
| `{rounded.lg}` | 0px | Template card thumbnails |
| `{rounded.xl}` | 0px | Pricing cards, mockup tiles |
| `{rounded.xxl}` | 0px | Gradient spotlight cards, oversized panels |
| `{rounded.pill}` | 0px | All primary text CTAs |
| `{rounded.full}` | 0px | Circular icon buttons, avatar circles |

### Photography & Illustration Geometry

- Embedded site mockups (browser-chromed previews) sit in sharp-cornered (`{rounded.xl}` = 0px) tiles with `{spacing.md}` 15px interior padding.
- Gradient spotlight cards are also sharp-cornered — no softening; the atmospheric feel comes from the saturated gradient against black canvas, not from corner radius.
- Icon glyphs and sub-nav glyphs render as sharp squares at 32–40px sizes.
...
- Compose every CTA as a sharp rectangle (`{rounded.pill}` = 0px); secondary actions live as charcoal rectangles, never as bordered ghost buttons.
...
- Don't add corner rounding. Sharp corners (`border-radius: 0`) are the brand vocabulary — the Athena override flattens the entire radius scale to 0px.
...
## Dashboard In-App Modal & Form Conventions

The dashboard modals follow the same dark system but diverge from the marketing site in corner treatment for functional surfaces.

### Modals

**`modal-backdrop`** — Full-screen overlay behind modals.
- Background `rgba(0, 0, 0, 0.65)`, backdrop-filter blur(8px), z-index 90.

**`modal-card`** — The primary modal container class (rounded).
- Background `{colors.surface-1}`, border `1px solid rgba(255, 255, 255, 0.12)`, rounded `{rounded.xxl}`, box-shadow `0 24px 80px rgba(0, 0, 0, 0.95)`.

### Corner Convention (site-wide override)

> **Athena override: `border-radius: 0` applies site-wide** — marketing CTAs,
> nav actions, cards, mockup tiles, spotlight panels, modals, form fields,
> selects, dropdowns and toggles are all sharp-cornered. There is no pill
> vocabulary left on the site; the only "rounded" element is the 1px
> translucent-blue focus/selection ring, which is a stroke, not a shape.
>
> The Framer-derived rule below is retained for reference only:

- **Marketing CTAs** and **dashboard button classes** (`btn-primary`, `btn-secondary`) remain pill-shaped (`{rounded.pill}`) per the brand spec — never change.
- **Dashboard modals for creator-facing data** (BalanceModal, CreatorInfoModal) now use `borderRadius: 0` (sharp corners) to distinguish them from the pill-shaped marketing CTA system.
- **Dashboard form fields, selects, and dropdowns** now use `borderRadius: 10` (`{rounded.md}`) with `boxShadow: 0 4px 14px rgba(0,0,0,0.35)` to match the sharp-corner modal buttons. This applies to:
  - `text-input` fields inside modals (SendClipForm, WithdrawForm)
  - `select` dropdowns
  - Date pickers
  - Pill-style toggle buttons inside forms (e.g., Bank/Crypto selector in WithdrawForm)

Rule of thumb: **pill = marketing CTA & nav actions; md (10px) = form controls inside modals; zero = modal container itself.**

### Buttons Inside Modals

Modal buttons use the same `btn-primary` / `btn-secondary` CSS classes as the rest of the dashboard — they remain pill-shaped (`{rounded.pill}`), no shadow, no extra border. The form field corners (10px) are intentionally sharper than the pill buttons they sit alongside, creating a clear hierarchy between input (sharper) and action (pill).
